...
interface ManifestEntry {
  ...
  hitCondition?: string;
  logMessage?: string;
  /** Free-text rationale. Ignored by the debug API; it documents the manifest. */
  note?: string;
}

interface Manifest {
  version?: number;
  breakpoints?: ManifestEntry[];
}

/** Stable identity for a breakpoint, so we only ever remove our own. */
function keyOf(fsPath: string, line: number): string {
  return `${fsPath}:${line}`;
}
...
function manifestUri(): vscode.Uri | undefined {
  ...
  const rel = vscode.workspace
    ...
    .get<string>("manifestPath", ".vscode/breakpoints.json");
 ...
 */
function resolveTarget(file: string): vscode.Uri | undefined {
...
async function readManifest(
  uri: vscode.Uri
): Promise<{ entries: ManifestEntry[]; problems: string[] }> {
  ...
  let parsed: Manifest | ManifestEntry[];
  try {
  ...
  for (const [index, entry] of list.entries()) {
    ...
    if (!Number.isInteger(entry.line) || entry.line < 1) {
      problems.push(
        `Entry ${index} (${entry.file}): "line" must be a positive integer, got ${entry.line}.`
      );
      continue;
    }
    entries.push(entry);
...
async function sync(
  ...
  if (!uri) {
    ...
    if (!quiet) {
      vscode.window.showWarningMessage(
        "Agent Breakpoints: no folder is open, so there is no manifest to read."
      );
    }
    return;
  }
  ...
  const desired = new Map<string, { entry: ManifestEntry; target: vscode.Uri }>();
  ...
  const owned = new Set(context.workspaceState.get<string[]>(OWNED_KEY, []));
  ...
  const stale = existing.filter((bp) =>
  ...
  for (const [key, { entry, target }] of desired) {
    ...
    toAdd.push(
      new vscode.SourceBreakpoint(
        new vscode.Location(target, position),
        entry.enabled ?? true,
        entry.condition,
        entry.hitCondition,
        entry.logMessage
  ...
  log(
    `sync done: desired=${desired.size} removed=${stale.length} added=${toAdd.length} ` +
  ...
  const summary = `Agent Breakpoints: ${desired.size} breakpoint${
    desired.size === 1 ? "" : "s"
  } synced from ${vscode.workspace.asRelativePath(uri)}.`;
...
async function clear(context: vscode.ExtensionContext): Promise<void> {
  const owned = new Set(context.workspaceState.get<string[]>(OWNED_KEY, []));
  ...
  vscode.window.setStatusBarMessage(
    `Agent Breakpoints: cleared ${mine.length} breakpoint${mine.length === 1 ? "" : "s"}.`,
    5000
  );
}
...
export function activate(context: vscode.ExtensionContext): void {
  ...
  log(
    `activate: manifestPath=${config.get("manifestPath")} autoSync=${config.get(
      "autoSync"
    )} syncOnStartup=${config.get("syncOnStartup")}`
  );

  context.subscriptions.push(
    vscode.commands.registerCommand("agentBreakpoints.sync", () => requestSync(context, "command")),
    vscode.commands.registerCommand("agentBreakpoints.diagnose", () => {
    ...
    vscode.commands.registerCommand("agentBreakpoints.clear", () => clear(context)),
    vscode.commands.registerCommand("agentBreakpoints.reveal", async () => {
      const uri = manifestUri();
      if (uri) {
  ...
  if (config.get<boolean>("autoSync", true)) {
    const rel = config.get<string>("manifestPath", ".vscode/breakpoints.json");
    const folder = vscode.workspace.workspaceFolders?.[0];
    if (folder) {
